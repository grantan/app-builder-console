...
namespace AppBuilderConsole.Utility
{
	class WebUtility
	{
		private const string NEWLINE_CARRIAGE = "\r\n";
		...
		private string WriteThing(Thing fullThing)
		{
			...
			{
				sb.Append(Tab(2));
				string propName = prop.PropertyName.Replace(" ", "");
				if (prop.IsList)
				{
					sb.Append("public List<" + prop.OwnedThing.Name + "> " + propName);
				}
				else
				{
					sb.Append("public " + prop.OwnedThing.Name + " " + propName);
				}
		...
		public string WriteThingWebFormAspx(string mapPath)
		{
			...
			success = WriteListAspxDesigner(webFolderPath);
		...
		private bool WriteListAspxCodeBehind(string webFolderPath)
		{
			...
			sb.Append(Method(true, 2, "protected", "void", "btnNew" + _thing.Name + "_Click", methodParams));
			sb.Append(TextValue("Response.Redirect(\"NewThing.aspx\");", 3));
		...
		private bool WriteListAspxDesigner(string webFolderPath)
		{
			StringBuilder sb = new StringBuilder();

			sb.Append(WriteAutoGeneratedTag());

			sb.Append(Namespace(true, _thing.Name, "Web"));
			sb.Append(PartialClass(true, _thing.Name + "List"));

			
			Dictionary<string, string> aspxControls = new Dictionary<string, string>();
			aspxControls.Add("global::System.Web.UI.HtmlControls.HtmlForm", "form1");
			aspxControls.Add("global::System.Web.UI.HtmlControls.GridView", "gv"+ _thing.Name);
			aspxControls.Add("global::System.Web.UI.HtmlControls.Button", "btnAdd" + _thing.Name);
			
			foreach (KeyValuePair<string, string> entry in aspxControls)
			{
			...
			string thingWebPath = _fileUtil.WriteFile(sb.ToString(), webFolderPath + "\\" + _thing.Name + "List.aspx.designer.cs");

			return !String.IsNullOrEmpty(thingWebPath);
		}

		private string WriteAutoGeneratedTag()
		{
			StringBuilder sb = new StringBuilder();
			sb.Append("//------------------------------------------------------------------------------");
			sb.Append(NEWLINE_CARRIAGE);
